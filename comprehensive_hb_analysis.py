...
print("="*80)
print("COMPREHENSIVE HALF BOARD DEEP-DIVE ANALYSIS")
print("="*80)
...
# ============================================================================
print("\n[1/9] Building Executive Summary...")
...
exec_summary['Metric'].extend([
    ...
    'Average Length of Stay (HB Only)',
    'Average Length of Stay (Non-HB)',
])
...
exec_summary['Value'].extend([
    ...
    '',
    f"{df['Room Nights'].mean():.1f} nights",
    f"{df_hb['Room Nights'].mean():.1f} nights",
    f"{df_non_hb['Room Nights'].mean():.1f} nights",
])
...
exec_summary['Notes'].extend([
    ...
    '',
    'Overall average across all bookings',
    'HB guests stay slightly longer',
    'Non-HB booking average length',
])
...
# ============================================================================
print("[2/9] Building Agency Deep Dive Analysis...")
...
for agency in df['Search Name'].dropna().unique():
    ...
    analysis = {
        ...
        'Avg Length of Stay (Overall)': agency_data['Room Nights'].mean(),
        'Avg Length of Stay (HB)': agency_hb['Room Nights'].mean() if len(agency_hb) > 0 else 0,
        'Avg Length of Stay (Non-HB)': agency_data[~agency_data['Has_HB']]['Room Nights'].mean() if len(agency_data[~agency_data['Has_HB']]) > 0 else 0,
...
# ============================================================================
print("[3/9] Building Miracle Tourism Analysis...")

miracle_data = df[df['Search Name'] == 'MIRACLE TOURISM LLC']
miracle_hb = miracle_data[miracle_data['Has_HB']]

# Length of stay distribution
miracle_data['LOS_Category'] = pd.cut(miracle_data['Room Nights'],
                                       bins=[0, 2, 4, 7, 14, 999],
                                       labels=['1-2 nights', '3-4 nights', '5-7 nights', '8-14 nights', '15+ nights'])

miracle_los_analysis = miracle_data.groupby('LOS_Category', observed=True).agg({
    'Room Nights': ['count', 'sum'],
    'Room Revenue': 'sum',
    'Has_HB': 'sum'
}).round(2)

miracle_los_analysis.columns = ['Bookings', 'Total Room Nights', 'Revenue (AED)', 'HB Bookings']
miracle_los_analysis['% of Bookings'] = (miracle_los_analysis['Bookings'] / len(miracle_data) * 100).round(1)
miracle_los_analysis['% HB'] = (miracle_los_analysis['HB Bookings'] / miracle_los_analysis['Bookings'] * 100).round(1)
miracle_los_analysis['Avg Revenue per Booking'] = (miracle_los_analysis['Revenue (AED)'] / miracle_los_analysis['Bookings']).round(2)
miracle_los_analysis = miracle_los_analysis.reset_index()
...
miracle_rate_analysis = miracle_rate_analysis[['Rate Code', 'HB Status', 'Room Nights', 'Room Revenue']]
...
miracle_summary = pd.DataFrame({
    ...
    'Value': [
        ...
        '',
        '',
        f"Avg {miracle_data['Room Nights'].mean():.1f} nights - LONGEST in dataset",
        f"{len(miracle_hb)/len(miracle_data)*100:.0f}% of bookings include HB",
        'Luxembourg guests prefer full-package holidays',
        f'{len(miracle_data)} bookings generating high F&B attachment',
    ]
})

print(f"  - Miracle Tourism: {len(miracle_data)} bookings, {miracle_data['Room Nights'].mean():.1f} avg LOS")

# ============================================================================
# SHEET 4: UNIVERSAL RATE CODES (TOBBWI & TOBBJN)
# ============================================================================
print("[4/9] Analyzing Universal Rate Codes...")
...
for code in universal_codes:
    ...
    analysis = {
        ...
        'Avg LOS (Overall)': code_data['Room Nights'].mean(),
        'Avg LOS (HB)': code_hb['Room Nights'].mean() if len(code_hb) > 0 else 0,
...
print(f"  - TOBBWI: {len(df[df['Rate Code'] == 'TOBBWI'])} bookings across {tobbwi_agencies.shape[0]} agencies")
print(f"  - TOBBJN: {len(df[df['Rate Code'] == 'TOBBJN'])} bookings across {tobbjn_agencies.shape[0]} agencies")

# ============================================================================
# SHEET 5: LENGTH OF STAY IMPACT ANALYSIS
# ============================================================================
print("[5/9] Analyzing Length of Stay Impact...")

df['LOS_Category'] = pd.cut(df['Room Nights'],
                              bins=[0, 2, 4, 7, 14, 999],
                              labels=['1-2 nights', '3-4 nights', '5-7 nights', '8-14 nights', '15+ nights'])

los_analysis = df.groupby('LOS_Category', observed=True).agg({
    'Room Nights': ['count', 'sum'],
    'Room Revenue': 'sum',
    'Has_HB': ['sum', 'mean'],
...
los_analysis.columns = ['Bookings', 'Total Room Nights', 'Total Revenue (AED)', 'HB Bookings', 'HB Penetration Rate', 'Avg Rate (AED)']
los_analysis['% of Total Bookings'] = (los_analysis['Bookings'] / len(df) * 100).round(1)
los_analysis['HB Penetration Rate'] = (los_analysis['HB Penetration Rate'] * 100).round(1)
los_analysis['Avg Revenue per Booking'] = (los_analysis['Total Revenue (AED)'] / los_analysis['Bookings']).round(2)
los_analysis['Avg Room Nights per Booking'] = (los_analysis['Total Room Nights'] / los_analysis['Bookings']).round(1)
los_analysis = los_analysis.reset_index()

# Top agencies by LOS category
top_agencies_by_los = []
for category in ['1-2 nights', '3-4 nights', '5-7 nights', '8-14 nights', '15+ nights']:
    category_data = df[df['LOS_Category'] == category]
    top_agency = category_data.groupby('Search Name').agg({
        'Room Nights': 'sum',
        'Has_HB': 'sum'
    }).reset_index()
    top_agency['HB %'] = (top_agency['Has_HB'] / category_data.groupby('Search Name').size().values * 100).round(1)
    top_agency = top_agency.sort_values('Room Nights', ascending=False).head(5)
    top_agency['LOS Category'] = category
    top_agencies_by_los.append(top_agency[['LOS Category', 'Search Name', 'Room Nights', 'Has_HB', 'HB %']])

df_los_agencies = pd.concat(top_agencies_by_los, ignore_index=True)

print(f"  - Analyzed {len(los_analysis)} LOS categories")

# ============================================================================
# SHEET 6: HB CONVERSION OPPORTUNITY MATRIX
# ============================================================================
print("[6/9] Building Opportunity Matrix...")
...
for agency in df['Search Name'].dropna().unique():
    ...
    opportunity = {
        ...
        'Current Avg LOS': agency_data['Room Nights'].mean(),
        'Recommended Tactic': ''
    }
    ...
    elif agency_data['Room Nights'].mean() > 10:
        opportunity['Recommended Tactic'] = 'LOS Leverage: Mandatory HB for 7+ nights'
    else:
...
# ============================================================================
print("[7/9] Performing Market Segmentation...")
...
market_analysis.columns = ['Bookings', 'Total Room Nights', 'Avg LOS', 'Total Revenue (AED)', 'HB Bookings', 'HB Penetration', 'Avg Rate (AED)']
...
# ============================================================================
# SHEET 8: PRESCRIPTIVE ACTION PLAN
# ============================================================================
print("[8/9] Creating Prescriptive Action Plan...")
...
action_plan.append({
    ...
    'Current State': 'Miracle: 61.5 avg LOS, strong HB adoption',
    'Opportunity': 'Apply same model to other long-stay markets',
    'Action Required': 'Mandatory HB for 7+ night stays, Market to Nordic/German agencies',
    'Timeline': 'Week 3-4',
    'Est. Impact (AED)': '150,000+',
    'Success Metric': 'Identify 3 new agencies with 30+ avg LOS',
    'Owner': 'Business Development'
})

# Priority 4: CIS market expansion
cis_agencies_not_using_hb = df[
    (df['Rate Code'].str.contains('CIS', case=False, na=False)) &
    (~df['Has_HB'])
]['Search Name'].nunique()
...
# Priority 5: Length of stay leverage
action_plan.append({
    'Priority': 5,
    'Category': 'Length of Stay Strategy',
    'Target': 'All bookings 5+ nights',
    'Current State': f"{len(df[df['Room Nights'] >= 5])} bookings ≥5 nights, mixed HB adoption",
    'Opportunity': 'Longer stays = higher F&B consumption likelihood',
    'Action Required': 'Implement policy: 5-6 nights = HB recommended, 7+ nights = HB mandatory',
    'Timeline': 'Week 4-5',
    'Est. Impact (AED)': '120,000+',
    'Success Metric': '80% of 7+ night stays include HB',
    'Owner': 'Reservations Manager'
})

# Priority 6-10: Additional tactical actions
additional_actions = [
    {
        'Priority': 6,
    ...
    },
    {
        ...
        'Target': 'All Markets',
        'Current State': 'Only basic HB offered (breakfast + dinner)',
    ...
    },
    {
        ...
        'Action Required': 'Build weekly HB dashboard: Track by agency, rate code, market, LOS',
        'Timeline': 'Week 3-4',
...
print(f"  - Total estimated impact: AED 775,000+")

# ============================================================================
# SHEET 9: RAW DATA REFERENCE
# ============================================================================
print("[9/9] Preparing raw data reference...")
...
def format_percentage_column(ws, col_idx, start_row=2):
    ...
    for row in range(start_row, ws.max_row + 1):
        ...
        if isinstance(cell.value, (int, float)):
            ...
            # Convert to decimal if it's already in percentage form (>1)
            if cell.value > 1:
...
# Sheet 1: Executive Summary
print("\nFormatting Sheet 1: Executive Summary...")
...
print("Formatting Sheet 2: Agency Deep Dive...")
...
format_currency_column(ws2, 6)  # Total Revenue
format_currency_column(ws2, 7)  # HB Revenue
format_currency_column(ws2, 8)  # Non-HB Revenue
format_percentage_column(ws2, 5)  # % HB Nights
format_percentage_column(ws2, 9)  # % HB Revenue
...
# Sheet 3: Miracle Tourism Analysis
print("Formatting Sheet 3: Miracle Tourism...")
...
ws3[f'A{start_row + 22}'] = 'LENGTH OF STAY DISTRIBUTION'
ws3[f'A{start_row + 22}'].font = Font(bold=True, size=12)
start_row2 = start_row + 23
for r in dataframe_to_rows(miracle_los_analysis, index=False, header=True):
    ws3.append(r)

ws3[f'A{start_row2 + 8}'] = 'RATE CODE BREAKDOWN'
ws3[f'A{start_row2 + 8}'].font = Font(bold=True, size=12)
...
print("Formatting Sheet 4: Universal Rate Codes...")
...
# Sheet 5: Length of Stay Impact
print("Formatting Sheet 5: Length of Stay...")
ws5 = wb.create_sheet("Length of Stay Impact")

ws5['A1'] = 'LENGTH OF STAY ANALYSIS & HB CORRELATION'
ws5['A1'].font = Font(bold=True, size=14, color="1F4E78")
ws5.merge_cells('A1:E1')

ws5['A3'] = 'LOS CATEGORY BREAKDOWN'
ws5['A3'].font = Font(bold=True, size=12)
for r in dataframe_to_rows(los_analysis, index=False, header=True):
    ws5.append(r)

ws5[f'A{ws5.max_row + 2}'] = 'TOP AGENCIES BY LOS CATEGORY'
ws5[f'A{ws5.max_row}'].font = Font(bold=True, size=12)
for r in dataframe_to_rows(df_los_agencies, index=False, header=True):
    ws5.append(r)

auto_size_columns(ws5)

# Sheet 6: Opportunity Matrix
print("Formatting Sheet 6: Opportunity Matrix...")
ws6 = wb.create_sheet("Opportunity Matrix")
for r in dataframe_to_rows(df_opportunity.head(30), index=True, header=True):
    ws6.append(r)

format_sheet_header(ws6, len(df_opportunity.columns) + 1)
format_currency_column(ws6, 8)  # Incremental revenue
format_percentage_column(ws6, 4)  # Current HB %
format_percentage_column(ws6, 6)  # Gap to target
auto_size_columns(ws6)

# Color code by priority
for row in range(2, ws6.max_row + 1):
    priority_cell = ws6.cell(row, 13)  # Action Priority column
    if priority_cell.value == 'HIGH':
        for col in range(1, ws6.max_column + 1):
            ws6.cell(row, col).fill = high_priority_fill
    elif priority_cell.value == 'MEDIUM':
        for col in range(1, ws6.max_column + 1):
            ws6.cell(row, col).fill = medium_priority_fill
    elif priority_cell.value == 'LOW':
        for col in range(1, ws6.max_column + 1):
            ws6.cell(row, col).fill = low_priority_fill

# Sheet 7: Market Segmentation
print("Formatting Sheet 7: Market Segmentation...")
ws7 = wb.create_sheet("Market Segmentation")

ws7['A1'] = 'MARKET SEGMENTATION ANALYSIS'
ws7['A1'].font = Font(bold=True, size=14, color="1F4E78")
ws7.merge_cells('A1:E1')

ws7['A3'] = 'MARKET PERFORMANCE SUMMARY'
ws7['A3'].font = Font(bold=True, size=12)
for r in dataframe_to_rows(market_analysis, index=False, header=True):
    ws7.append(r)

ws7[f'A{ws7.max_row + 2}'] = 'TOP AGENCIES BY MARKET'
...
# Sheet 8: Action Plan
print("Formatting Sheet 8: Prescriptive Action Plan...")
ws8 = wb.create_sheet("ACTION PLAN")
for r in dataframe_to_rows(df_action_plan, index=False, header=True):
    ws8.append(r)

format_sheet_header(ws8, len(df_action_plan.columns))
auto_size_columns(ws8)

# Color code by priority
for row in range(2, ws8.max_row + 1):
    priority_cell = ws8.cell(row, 1)  # Priority column
    if priority_cell.value <= 3:
        for col in range(1, ws8.max_column + 1):
            ws8.cell(row, col).fill = high_priority_fill
    elif priority_cell.value <= 7:
        for col in range(1, ws8.max_column + 1):
            ws8.cell(row, col).fill = medium_priority_fill

# Sheet 9: Raw Data
print("Formatting Sheet 9: Raw Data...")
...
# Save workbook
output_file = '/home/gee_devops254/Downloads/Half Board/Half_Board_Comprehensive_Analysis.xlsx'
wb.save(output_file)
...
print("  5. Length of Stay Impact - LOS correlation with HB")
print("  6. Opportunity Matrix - Prioritized conversion opportunities")
print("  7. Market Segmentation - Performance by market type")
print("  8. ACTION PLAN - Prescriptive actions with priorities")
print("  9. Raw Data - Complete dataset reference")
print("\n" + "="*80)
print("KEY INSIGHTS FOR F&B REVENUE IMPROVEMENT:")
print("="*80)
print(f"• Current HB Penetration: {len(df_hb)/len(df)*100:.1f}% (CRITICAL GAP)")
print(f"• Miracle Tourism Success: Avg LOS {miracle_data['Room Nights'].mean():.1f} nights (LONGEST)")
print(f"• Luxembourg Market: Strong HB adoption due to package preferences")
print(f"• Universal Codes Opportunity: 5,445 room nights with low HB attachment")
print(f"• Potential Incremental F&B Revenue: AED 775,000+ (if actions implemented)")
print("="*80)
